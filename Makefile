...
run:
	@docker run \
	  ...
	  -v $(shell pwd)/var/lib/cobbler/kickstarts:/var/lib/cobbler/kickstarts \
	  -v $(shell pwd)/var/lib/cobbler/snippets:/var/lib/cobbler/snippets \
...
check:
	@docker exec -it cobbler systemctl status httpd cobblerd dhcpd
